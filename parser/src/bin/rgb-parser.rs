...
fn hex_to_u8(s: &str) -> IResult<&str, u8> {
    map_res(take_while_m_n(2, 2, |ch| is_hex_digit(ch as u8)), |x| {
...
fn rbg(s: &str) -> IResult<&str, u32> {
    preceded(
        char('#'),
        map(count(hex_to_u8, 3), |hex| {
            (hex[0] as u32) << 16 | (hex[1] as u32) << 8 | hex[2] as u32
        }),
    )(s)
}
...
fn main() {
    ...
    assert_eq!(
        rbg("$FFFFFF"),
        Err(Err::Error(("$FFFFFF", ErrorKind::Char)))
    );
    assert_eq!(
        rbg("#FFFGHI"),
        Err(Err::Error(("FGHI", ErrorKind::TakeWhileMN)))
    );
